...
def main(args):
    ...
    if args.split_data == True:
            ...
            if Path.exists(Path(local_lookup_dir + "lookup_train.csv")):
                ...
                overwrite = input(f"You passed `--split_data` but lookup tables already exists at {local_lookup_dir}. Overwrite? (y/n): ")
            ...
            df = pd.read_csv(main_lookup_dir + "main_lookup.csv", delimiter=",").sample(frac=1., random_state=np.random.RandomState(42))
            n_total = len(df)
            ...
            train_df.to_csv(local_lookup_dir + "lookup_train.csv", sep=",", index=False)
            test_df.to_csv(local_lookup_dir + "lookup_test.csv", sep=",", index=False)
            val_df.to_csv(local_lookup_dir + "lookup_val.csv", sep=",", index=False)
    ...
    for img_width in img_widths:
        for randomize in randomize_flags:
            for sample_frac in sample_fracs:
                ...
                logger = pl.loggers.TensorBoardLogger(log_dir+f'/sample_frac_{sample_frac:2.2f}/img_width_{img_width}/randomized_{randomize}/', name + f"_sample_frac_{sample_frac:2.2f}")
                ...
                if param_min and param_max:
                    ...
                    loss_weights = (10./param_widths).tolist() # shift and scale factor to put model outputs / labels in [-5,5] range
                else:
